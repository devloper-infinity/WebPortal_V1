...
namespace WebPortal.Admin
{
    public partial class AttendanceCorrectionpm : System.Web.UI.Page
    {
        ...
        {
            DataTable dt1 = new bllMaster().GetAllStandardReasonsForPM();
            List<Dictionary<string, object>> rows = new List<Dictionary<string, object>>();
            Dictionary<string, object> row;
            foreach (DataRow dr in dt1.Rows)
            {
        ...
        {
            int returnvalue = 0;
            returnvalue = new bllMaster().GetAttendanceRequestCount(Code);
            return returnvalue;
        }
        ...
        {
            DataTable dt1 = new bllMaster().GetAllAttendanceCorrectionRequestForPM(int.Parse(HttpContext.Current.User.Identity.Name.ToString()));
            List<Dictionary<string, object>> rows = new List<Dictionary<string, object>>();
            Dictionary<string, object> row;
            foreach (DataRow dr in dt1.Rows)
            {
        ...
        {
            DataTable dt1 = new bllMaster().GetAllDateForAttendance(Code);
            List<Dictionary<string, object>> rows = new List<Dictionary<string, object>>();
            Dictionary<string, object> row;
            foreach (DataRow dr in dt1.Rows)
            {
                    ...
                    if (LogIntime1 == timeIn12HourFormatForDisplay)
                    {
                        ...
                        if (OutDateParam != "" && newLogout != "" && OutTimeConvention != "")
                        {
                            ...
                            if (OutDate1 != "Select" && OutDateParam != "" && LogOutTime != null && OutTimeParam != null && OutTimeParam != "" && OutTimeConvention != "")
                            {
                                ...
                                if (OutDateTime > InDateTime)
                                {
                                    ...
                                    htAttendance.Add("PrevDate", "");
                                    htAttendance.Add("IpAddress", "23.111.175.186");
                                    htAttendance.Add("ApprovedBy", int.Parse(HttpContext.Current.User.Identity.Name.ToString()));

                                    returnvalue = new bllMaster().InsertAttendanceCorrectRequestByPM(htAttendance);

                                    if (returnvalue > 0)
                                    {
                                        SendAttendanceEmail_InOut(Code, InDateParam, InTime, OutDateParam, OutTime, TotalHours, UserReason, reasontypeParam, userreasonParam);
                                    }
                                    else
                                    {
                                        returnvalue = 0;
                        ...
                        else
                        {
                            ...
                            if (InTime != "" && OutDateParam == "" && OutTimeConvention == "" || OutTime != "")//|| BreakInTime != "" || BreakOutTime != "")
                            {
                                ...
                                if (returnvalue > 0)
                                {
                                    SendAttendanceEmail_InOut(Code, InDateParam, InTime, "", "", TotalHours, UserReason, reasontypeParam, userreasonParam);
                                }
                                else
                                {
                                    returnvalue = 0;
