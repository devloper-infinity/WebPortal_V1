...
<body>
    ...
    <script>
let assignment=0,questions=[],loadingCount=0;function showLoading(text){loadingCount++;document.getElementById('rnrLoadingText').textContent=text||'Please wait...';document.getElementById('rnrLoading').classList.add('show')}function hideLoading(){loadingCount=Math.max(0,loadingCount-1);if(!loadingCount)document.getElementById('rnrLoading').classList.remove('show')}function localLoading(text){showLoading(text);setTimeout(hideLoading,300)}async function call(n,a,text){showLoading(text||'Processing...');try{let r=await fetch('RNRFeedback.aspx/'+n,{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json','Cache-Control':'no-cache'},body:JSON.stringify(a||{})}),x;try{x=await r.json()}catch(_){throw Error('The ERP returned an invalid response. Please sign in again and retry.')}if(!r.ok)throw Error(x.Message||'Request failed');return x.d}finally{hideLoading()}}function e(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
async function init(){let x=await call('Load',{},'Loading questionnaire...');if(!x){window.location.replace('../Login.aspx');return}assignment=x.Header.AssignmentID;questions=x.Questions;let h=`<div class="card hero"><h1>${e(x.Header.Title)}</h1><p>${e(x.Header.Description)}</p><p class="period">${e(x.Header.SurveyYear)} · ${e(x.Header.Quarter)}</p></div>`;questions.forEach(q=>{let opts=x.Options.filter(o=>o.QuestionID===q.QuestionID);h+=`<div class="card" data-id="${q.QuestionID}" data-type="${q.QuestionType}" data-required="${q.IsRequired}"><div class="q">${e(q.QuestionText)} ${q.IsRequired?'<span class="req">*</span>':''}</div>`;if(q.QuestionType==='Text')h+='<textarea></textarea>';else if(q.QuestionType==='Rating'){for(let i=q.RatingMin;i<=q.RatingMax;i++)h+=`<label class="choice"><input type="radio" name="q${q.QuestionID}" value="${i}">${i}</label>`}else{if(q.QuestionType==='YesNo')opts=[{OptionID:-1,OptionText:'Yes'},{OptionID:-2,OptionText:'No'}];opts.forEach(o=>h+=`<label class="choice"><input type="${q.QuestionType==='MultipleChoice'?'checkbox':'radio'}" name="q${q.QuestionID}" value="${o.OptionID}">${e(o.OptionText)}</label>`)}h+='</div>'});h+='<div class="card"><div id="error" class="error"></div><button type="button" onclick="submitForm()">Submit feedback</button></div>';document.getElementById('form').innerHTML=h}
async function submitForm(){localLoading('Validating feedback...');let answers=[],bad=false;document.querySelectorAll('[data-id]').forEach(d=>{let type=d.dataset.type,a={QuestionID:+d.dataset.id,OptionIDs:[]};if(type==='Text')a.Text=d.querySelector('textarea').value.trim();else if(type==='Rating'){let c=d.querySelector(':checked');if(c)a.Rating=+c.value}else if(type==='YesNo'){let c=d.querySelector(':checked');if(c)a.Text=c.value==='-1'?'Yes':'No'}else a.OptionIDs=[...d.querySelectorAll(':checked')].map(x=>+x.value);let supplied=a.Text||a.Rating||a.OptionIDs.length;if(d.dataset.required==='true'&&!supplied){d.style.border='1px solid #dc2626';bad=true}else d.style.border='';answers.push(a)});if(bad){document.getElementById('error').textContent='Please answer every required question.';return}try{await call('Submit',{assignmentId:assignment,answers:answers},'Submitting feedback...');window.location.replace('DashboardEmployee.aspx')}catch(ex){document.getElementById('error').textContent=ex.message}}
init();</script>
</body>
</html>
