...
namespace WebPortal.Admin
{
 public partial class RNRReport:System.Web.UI.Page
 {
  ...
  [WebMethod] public static object Lookups(){RnrSecurity.RequireHr();var s=new RnrFeedbackRepository().GetAdminData();return new{Questionnaires=Rows(s.Tables[0]),Domains=Rows(s.Tables[1]),Locations=Rows(s.Tables[2]),Departments=Rows(s.Tables[3])};}
  [WebMethod] public static object Report(int questionnaire,string domain,int location,int department,int employee,string status,string from,string to){RnrSecurity.RequireHr();var s=new RnrFeedbackRepository().Report(questionnaire,domain,location,department,employee,status,Date(from),Date(to));return new{Rows=Rows(s.Tables[0]),Counts=Rows(s.Tables[1]).FirstOrDefault()};}
