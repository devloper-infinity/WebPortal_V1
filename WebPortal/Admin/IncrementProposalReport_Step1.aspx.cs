...
{
    public partial class IncrementProposalReport_Step1 : System.Web.UI.Page
    {
        ...
        public static int InsertIncrementProposalRecord(string Codes)
        {
            ...
            foreach (string item in codeList)
            {
                ...
                returnValue = new bllSalary().InsertIncrementDue_Step1(htParam);
            }
        ...
        private void DownloadPerformanceDocument(string token)
        {
            try
            {
                string filePath = DecodeToken(token);
