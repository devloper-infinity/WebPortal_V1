        ...
        <div class="rnr-card">
            ...
            <button type="button" onclick="assign()">Publish to selected</button><table id="employees">
                <thead>
                    <tr>
                        <th>
                            <input type="checkbox" onclick="localLoading('Updating selection...');document.querySelectorAll('.emp').forEach(x=>x.checked=this.checked)" /></th>
                        <th>ID</th>
                        <th>Name</th>
                        <th>Domain</th>
                        <th>Location</th>
...
async function init(){try{data=await call('Load',{},'Loading R & R administration...');fill('questionnaires',data.Questionnaires,'QuestionnaireID','Title',true);fill('assignQuestionnaire',data.Questionnaires.filter(x=>x.Status==='Published'&&x.IsActive),'QuestionnaireID','Title');fill('domain',data.Domains,'Value','Text',true,'');fill('location',data.Locations,'Value','Text',true);fill('department',data.Departments,'Value','Text',true);let now=new Date(),y=now.getFullYear(),years=[];for(let i=y+1;i>=y-5;i--)years.push({Value:i,Text:i});fill('surveyYear',years,'Value','Text',false);document.querySelector(`#quarters input[value="${1<<Math.floor(now.getMonth()/3)}"]`).checked=true}catch(e){document.getElementById('msg').textContent=e.message}}
 function fill(id,a,v,t,all,allValue){document.getElementById(id).innerHTML=(all?option(allValue===undefined?0:allValue,'All / select'):'')+a.map(x=>option(x[v],x[t])).join('')}
...
async function setActive(){localLoading('Validating selection...');try{if(!editing)throw Error('Select a questionnaire first.');await call('SetActive',{questionnaireId:editing,active:document.getElementById('active').checked},'Updating questionnaire state...');document.getElementById('msg').textContent='Questionnaire state updated.'}catch(e){document.getElementById('msg').textContent=e.message}}
 async function loadEmployees(button){let message=document.getElementById('msg');try{message.textContent='Loading employees...';if(button)button.disabled=true;let locationValue=parseInt(document.getElementById('location').value,10),departmentValue=parseInt(document.getElementById('department').value,10),a=await call('Employees',{domain:document.getElementById('domain').value||'',location:Number.isFinite(locationValue)?locationValue:0,department:Number.isFinite(departmentValue)?departmentValue:0},'Loading employees...');if(window.jQuery&&$.fn.DataTable&&$.fn.DataTable.isDataTable('#employees'))$('#employees').DataTable().clear().destroy();let body=document.querySelector('#employees tbody');body.innerHTML=a.length?a.map(x=>`<tr><td><input class="emp" type="checkbox" value="${x.EmployeeID}"></td><td>${esc(x.EmployeeCode)}</td><td>${esc(x.EmployeeName)}</td><td>${esc(x.DomainName)}</td><td>${esc(x.Location)}</td><td>${esc(x.Department)}</td></tr>`).join(''):'<tr><td colspan="6" class="text-center text-muted">No active employees found for the selected filters.</td></tr>';if(a.length&&window.jQuery&&$.fn.DataTable)$('#employees').DataTable({responsive:true,pageLength:1200,order:[[2,'asc']]});message.textContent=a.length+' employee(s) loaded.'}catch(e){message.textContent='Unable to load employees: '+e.message}finally{if(button)button.disabled=false}}
