...
namespace WebPortal.Admin
{
    public partial class RNRAdmin : System.Web.UI.Page
    {
        ...
        [WebMethod] public static object Load() { RnrSecurity.RequireHr(); var s = new RnrFeedbackRepository().GetAdminData(); return new { Questionnaires = Rows(s.Tables[0]), Domains = Rows(s.Tables[1]), Locations = Rows(s.Tables[2]) }; }
        ...
        [WebMethod] public static object Employees(int? domain, int? location) { RnrSecurity.RequireHr(); return Rows(new RnrFeedbackRepository().Employees(domain.GetValueOrDefault(), location.GetValueOrDefault())); }
