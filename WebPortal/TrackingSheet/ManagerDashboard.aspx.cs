...
namespace WebPortal.TrackingSheet
{
    public partial class ManagerDashboardPage : System.Web.UI.Page
    {
        ...
        [WebMethod] public static string GetDeals(int projectId) { ProjectIds(projectId); return OLTrackingWeb.Json(new bllOLTracking().GetSourceDeals(projectId, OLTrackingWeb.UserId)); }
        [WebMethod] public static string GetDealDashboard(int projectId, string dealNumber) { ProjectIds(projectId); return OLTrackingWeb.Json(new bllOLTracking().GetDealDashboard(projectId, dealNumber)); }
        [WebMethod]
        ...
        [WebMethod] public static string GetReallocationUsers(int projectId, string dealNumber, int processId) { ProjectIds(projectId); return OLTrackingWeb.Json(new bllOLTracking().GetReallocationUsers(projectId, dealNumber, processId)); }
        [WebMethod] public static string GetReallocationOrders(int projectId, string dealNumber, int processId, int fromUserId) { ProjectIds(projectId); return OLTrackingWeb.Json(new bllOLTracking().GetReallocationOrders(projectId, dealNumber, processId, fromUserId)); }
        [WebMethod]
        public static ReallocationResult ReallocateOrders(int projectId, int fromUserId, int toUserId, long[] assignmentIds, string remark)
        {
            try
            {
                ProjectIds(projectId); int count = new bllOLTracking().ReallocateOrders(projectId, fromUserId, toUserId, assignmentIds, remark, OLTrackingWeb.UserId);
                return new ReallocationResult { Success = count > 0, ReallocatedCount = count, Message = count + " order(s) re-allocated successfully." };
            }
            catch (Exception ex)
            {
                SqlException sql = ex as SqlException; string message = sql != null && sql.Number >= 50110 && sql.Number <= 50133 ? sql.Message : "The selected orders could not be re-allocated.";
        ...
        private static int[] ProjectIds(int selected)
        {
            ...
            if (selected != 0) { if (!ids.Contains(selected)) throw new UnauthorizedAccessException("Project access denied."); return new[] { selected }; } return ids.ToArray();
        }
