        ...
        <div class="olt-dialog olt-dialog-large" role="dialog" aria-modal="true" aria-labelledby="completeDialogTitle">
            ...
            <div class="olt-dialog-body">
                ...
                <div id="statusStep" class="ots-step">
                    <div class="olt-form">
                        <div class="olt-field wide"><label>Status</label><select id="updateStatus"><option value="">Select status</option><option value="Completed">Completed</option></select></div>
                        <div class="olt-field full olt-actions"><button type="button" class="olt-btn" onclick="selectCompletionStatus()">Continue</button></div>
                    </div>
                </div>
...
function openComplete(id){activeAssignment=queue.filter(function(q){return q.AssignmentID===id;})[0]||null;feedbackRequired=!!(activeAssignment&&activeAssignment.FeedbackRequiredOnComplete);savedFeedbacks=0;assignmentId.value=id;completeRemark.value='';updateStatus.value='';savedFeedbackList.innerHTML='';savedFeedbackCount.textContent='0 feedback added';continueAfterFeedback.disabled=true;[statusStep,feedbackStep,completeStep].forEach(function(s){s.classList.remove('active');});statusStep.classList.add('active');completeModal.classList.add('open');}
function closeComplete(){completeModal.classList.remove('open');activeAssignment=null;}
function selectCompletionStatus(){if(updateStatus.value!=='Completed'){OLT.alert('Please select Completed status.',true);return;}statusStep.classList.remove('active');if(!feedbackRequired){completeStep.classList.add('active');return;}feedbackStep.classList.add('active');loadFeedbackForm();}
