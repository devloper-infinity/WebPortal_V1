...
namespace WebPortal.Assets
{
    public partial class AddAsset : System.Web.UI.Page
    {
        ...
        [WebMethod] public static object List() { return AssetBAL.Rows(S.List("Asset")); }
        [WebMethod] public static object Get(long id) { return AssetBAL.Sets(S.Detail("Asset", id)); }
        [WebMethod] public static ApiResult Save(AssetInput x) { try { return ApiResult.Ok(S.SaveAsset(x, U)); } catch (Exception ex) { return ApiResult.Fail(ex.Message); } }
    }
}