...
IF COL_LENGTH('dbo.IHMS_Ticket','DeskNo') IS NULL ALTER TABLE dbo.IHMS_Ticket ADD DeskNo nvarchar(80) NULL;
...
BEGIN SET NOCOUNT ON; SET XACT_ABORT ON; BEGIN TRAN;
 ...
 INSERT dbo.IHMS_Ticket(RequestorEmployeeID,RequestorName,RequestorEmail,Department,Location,DeskNo,Subject,Description,RequestTypeID,PriorityID,StatusID,Source,ApprovalRequired,SLADueDate,GraphMessageID,InternetMessageID,CreatedBy)
 VALUES(@RequestorEmployeeID,@RequestorName,@RequestorEmail,@Department,@Location,@DeskNo,@Subject,@Description,@RequestTypeID,@PriorityID,@StatusID,@Source,@Approval,CASE WHEN @resolution IS NULL THEN NULL ELSE DATEADD(MINUTE,@resolution,SYSDATETIME()) END,@GraphMessageID,@InternetMessageID,@CreatedBy);
...
BEGIN SET NOCOUNT ON; DECLARE @isIT bit=CASE WHEN dbo.IHMS_fn_HasRole(@EmployeeID,'ITUser')=1 OR dbo.IHMS_fn_HasRole(@EmployeeID,'ITHead')=1 OR dbo.IHMS_fn_HasRole(@EmployeeID,'HelpdeskAdmin')=1 THEN 1 ELSE 0 END;
 ...
 SELECT v.*,t.RequestTypeID,t.DeskNo,t.Description,t.ApprovalRequired,t.FirstRespondedDate FROM dbo.IHMS_vw_TicketSummary v JOIN dbo.IHMS_Ticket t ON t.TicketID=v.TicketID WHERE v.TicketID=@TicketID;
...
BEGIN SET NOCOUNT ON;
 ...
 SELECT i.TicketID,(SELECT TOP 1 OfficialEmailID FROM dbo.EmployeeInfo WHERE EmployeeID=CONVERT(int,i.NewValue)),'['+t.TicketNo+'] Approval required - '+t.Subject,'A helpdesk ticket requires your decision.<br/>Ticket: '+t.TicketNo+'<br/>'+COALESCE(i.Details,'')
...
GO
IF OBJECT_ID('dbo.IHMS_Ticket_ChangeLocation','P') IS NOT NULL DROP PROCEDURE dbo.IHMS_Ticket_ChangeLocation;
GO
CREATE PROCEDURE dbo.IHMS_Ticket_ChangeLocation @TicketID bigint,@Location nvarchar(150),@EmployeeID int AS
BEGIN SET NOCOUNT ON; IF dbo.IHMS_fn_HasRole(@EmployeeID,'ITUser')=0 AND dbo.IHMS_fn_HasRole(@EmployeeID,'ITHead')=0 AND dbo.IHMS_fn_HasRole(@EmployeeID,'HelpdeskAdmin')=0 THROW 51003,'IT role required.',1; IF EXISTS(SELECT 1 FROM dbo.IHMS_Ticket t JOIN dbo.IHMS_TicketStatus s ON s.StatusID=t.StatusID WHERE t.TicketID=@TicketID AND s.StatusCode='PendingApproval') THROW 51004,'Ticket is locked pending approval.',1; DECLARE @old nvarchar(150); SELECT @old=Location FROM dbo.IHMS_Ticket WHERE TicketID=@TicketID; UPDATE dbo.IHMS_Ticket SET Location=@Location,ModifiedBy=@EmployeeID,ModifiedDate=SYSDATETIME() WHERE TicketID=@TicketID; INSERT dbo.IHMS_TicketActivity(TicketID,ActivityType,OldValue,NewValue,PerformedBy) VALUES(@TicketID,'Location',@old,@Location,@EmployeeID); END
GO
