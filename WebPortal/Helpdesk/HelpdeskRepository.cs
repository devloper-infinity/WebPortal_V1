...
namespace WebPortal.Helpdesk
{
    internal sealed class HelpdeskRepository
    {
        ...
        public DataSet Ticket(long ticketId,int employeeId) { var c=Command("IHMS_Ticket_Get");Add(c,"@TicketID",ticketId);Add(c,"@EmployeeID",employeeId);return Query(c); }
        ...
        { var c=Command("IHMS_Ticket_List");Add(c,"@EmployeeID",employeeId);Add(c,"@Scope",scope);Add(c,"@StatusCode",Empty(status));Add(c,"@TicketNo",Empty(ticketNo));Add(c,"@FromDate",from);Add(c,"@ToDate",to);Add(c,"@RequestTypeID",requestType);Add(c,"@PriorityCode",Empty(priority));Add(c,"@AssignedTo",assigned);Add(c,"@Source",Empty(source));Add(c,"@ApprovalStatus",Empty(approval));return Query(c).Tables[0]; }
        public DataTable Create(int employeeId,string name,string email,string department,string location,string subject,string description,int? requestType,string priority)
        { var c=Command("IHMS_Ticket_Create");Add(c,"@RequestorEmployeeID",employeeId);Add(c,"@RequestorName",name);Add(c,"@RequestorEmail",email);Add(c,"@Department",department);Add(c,"@Location",location);Add(c,"@Subject",subject);Add(c,"@Description",description);Add(c,"@RequestTypeID",requestType);Add(c,"@PriorityCode",priority);Add(c,"@Source","Portal");Add(c,"@CreatedBy",employeeId);var output=c.Parameters.Add("@TicketID",SqlDbType.BigInt);output.Direction=ParameterDirection.Output;return Query(c).Tables[0]; }
        ...
        public DataTable Performance(DateTime from,DateTime to,int? employee,int? type,string priority){var c=Command("IHMS_Performance");Add(c,"@FromDate",from);Add(c,"@ToDate",to);Add(c,"@EmployeeID",employee);Add(c,"@RequestTypeID",type);Add(c,"@PriorityCode",Empty(priority));return Query(c).Tables[0];}
