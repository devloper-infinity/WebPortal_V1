...
namespace WebPortal.SoftwareRequests
{
 public partial class NewRequest : SoftwareRequestBasePage
 {
  ...
  [WebMethod] public static long CreateRequest(int requestTypeId,int applicationId,int moduleId,string title,string description,string justification,string requestedPriority,DateTime? requiredBy){return Dashboard.CreateRequest(requestTypeId,applicationId,moduleId,title,description,justification,requestedPriority,requiredBy,new SoftwareRequestRepository().EmployeeCode(CurrentEmployeeID),"",null);}
